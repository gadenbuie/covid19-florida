...
combine_scraped_and_api_data() %>% 
  filter(day %in% (today - c(0, 1, 7, 14))) %>% 
  arrange(desc(day)) %>% 
  mutate(
    when = case_when(
      day == today ~ "Today",
      day == today - 1 ~ "Yesterday",
      day == today - 7 ~ "Last Week",
      day == today - 14 ~ "2 Weeks Ago"
    )
  ) %>% 
  select(when, day, positive, deaths, negative, inconclusive, total, pending) %>% 
