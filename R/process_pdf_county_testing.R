process_county_testing <- function(page_text, timestamp) {
  ...
  } else if (timestamp >= "2020-03-25 10:07:00 EDT" && timestamp < "2020-04-11 17:25:00 EDT") {
    process_county_testing_v4(page_text, timestamp)
  } else if (timestamp >= "2020-04-11 17:25:00 EDT") {
    process_county_testing_v4(page_text, timestamp)
  }
}
...
process_county_testing_v4 <- function(page_text, timestamp) {
  ...
  page_text[pages_testing] %>%
    ...
    drop_empty() %>%
    select(-contains("percent")) %>% 
    mutate_at(vars(pending, inconclusive, negative, positive), str_remove_all, pattern = "\\s*,\\s*") %>% 
...
process_county_testing_v5 <- function(page_text, timestamp) {
  pages_testing <- page_text %>% 
    map_lgl(str_detect, pattern = "All persons with test") %>% 
    which()
  ...
  page_text[pages_testing] %>%
    ...
    str_replace_all("[\\d,]+\\s+\\d+%\n", "\n") %>% 
    read_table_pages(
      col_names = c("county", "pending", "inconclusive", "negative", "positive", "percent", "total"),
      pattern_tab = "\\s{3,}"
    ) %>% 
    filter(county != "Total") %>% 
    drop_empty() %>%
    select(-contains("percent"), -contains("total")) %>% 
    mutate_at(vars(pending, inconclusive, negative, positive), str_remove_all, pattern = "\\s*,\\s*") %>% 
    mutate_at(vars(pending, inconclusive, negative, positive), as.numeric) %>% 
    replace_na(list(inconclusive = 0)) %>% 
    add_total(from = c("negative", "positive", "inconclusive")) %>%  
